...
public class GvrReticlePointer : GvrBasePointer {
  ...
  public float RETICLE_DISTANCE_MIN = 0.5f;
  // Maximum distance of the reticle (in meters).
  public float RETICLE_DISTANCE_MAX = 20.0f;

  public float RETICLE_DISTANCE_Default = 5.0f;
  ...
  public override float MaxPointerDistance { get { return RETICLE_DISTANCE_MAX; } }
    ...
    /// If true then Pointer distance will remain at its last distance if no target is present.
    /// (Created by Sumit Chaudhary)
    [Tooltip("If true then Gaze distance will remain at its last distance if no target is present.")]
    public bool MaintainDistanceOnExit = true;

    /// If true then Pointer distance from camera will not change.
    /// (Created by Sumit Chaudhary)
    [Tooltip("If true then Gaze distance from camera will not change")]
  ...
  public override void OnPointerExit(GameObject previousObject) {
        if (!LockDistance && !MaintainDistanceOnExit)
        {
            ReticleDistanceInMeters = RETICLE_DISTANCE_Default;
        }
  ...
  public override void GetPointerRadius(out float enterRadius, out float exitRadius) {
    ...
    enterRadius = 0.4f * Mathf.Tan(min_inner_angle_radians);
    exitRadius = 0.5f * Mathf.Tan(max_inner_angle_radians);
  }
  ...
  public void UpdateDiameters() {
    ...
    float inner_diameter = 0.8f * Mathf.Tan(inner_half_angle_radians);
    float outer_diameter = 1.0f * Mathf.Tan(outer_half_angle_radians);
